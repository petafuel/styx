...
public abstract class AbstractTokenFilter implements ContainerRequestFilter {
    ...
    public void filter(ContainerRequestContext context) {
        ...
        if (token == null || "".equals(token)) {
            ErrorEntity errorEntity = new ErrorEntity("Token was not contained or empty in request", Response.Status.BAD_REQUEST, ErrorCategory.STYX);
            errorEntity.setCategory(ErrorCategory.STYX);
        ...
        try {
            ...
            ErrorEntity errorEntity = new ErrorEntity("Token value has an invalid format", Response.Status.BAD_REQUEST, ErrorCategory.STYX);
            errorEntity.setCategory(ErrorCategory.STYX);
            throw new StyxException(errorEntity);
        }
        boolean tokenValid = checkToken(uuid);
        if (!tokenValid) {
            ErrorEntity errorEntity = new ErrorEntity("Token is invalid", Response.Status.UNAUTHORIZED, ErrorCategory.STYX);
            errorEntity.setCategory(ErrorCategory.STYX);
            throw new StyxException(errorEntity);
        }
    }
