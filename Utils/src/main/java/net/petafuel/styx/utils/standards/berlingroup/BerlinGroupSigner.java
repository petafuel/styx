...
public class BerlinGroupSigner implements Signer
{
    ...
    public BerlinGroupSigner()
    {
        ...
        {
            // @TODO Richtiges Errorhandling implementieren
            e.printStackTrace();
        }
    }
    ...
    public void sign(XS2ARequest xs2aRequest)
    {
        ...
        try
        {
            this.checkMandatoryHeaders(xs2aRequest);
        } catch (Exception e)
        {
            e.printStackTrace();
        }
    ...
    private void checkMandatoryHeaders(XS2ARequest request) throws Exception
    {
        LinkedHashMap<String, String> headers = request.getHeaders();

        //TODO better errorhandling
        if (headers.get(HEADER_DIGEST) == null || headers.get(HEADER_DIGEST).isEmpty())
        {
            throw new Exception(HEADER_DIGEST + " Header must be present in request to generate a signature");
        }
        else if (headers.get(HEADER_X_REQUEST_ID) == null || headers.get(HEADER_X_REQUEST_ID).isEmpty())
        {
            throw new Exception(HEADER_X_REQUEST_ID + " Header must be present in request to generate a signature");
        }
        else if (headers.get(HEADER_DATE) == null || headers.get(HEADER_DATE).isEmpty())
        {
            throw new Exception(HEADER_DATE + " Header must be present in request to generate a signature");
        }
    }
}
