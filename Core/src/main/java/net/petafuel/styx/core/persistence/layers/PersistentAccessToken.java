...
public class PersistentAccessToken {
     ...
     * @param service         - service should be one of ais, pis, piis, aispis
     * @param expiresIn       - default is 300
     * @return new access token + metadata
     */
    public static AccessToken create(String masterTokenHash, String accessTokenHash, AccessToken.ServiceType service, Integer expiresIn) {
        Connection connection = Persistence.getInstance().getConnection();
        AccessToken model = new AccessToken();
        try (PreparedStatement query = connection.prepareStatement("SELECT * FROM create_token(?, ?, ?, ?);")) {
            ...
            query.setString(3, service.toString());
    ...
    private static AccessToken dbToModel(ResultSet resultSet) throws SQLException {
        ...
        model.setService(AccessToken.ServiceType.getByString(resultSet.getString("service")));
