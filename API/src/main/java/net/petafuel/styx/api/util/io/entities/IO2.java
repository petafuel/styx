 ...
 */
public class IO2 implements ApplicableImplementerOption<IOInputContainerPIS> {
    ...
    public void apply(IOInputContainerPIS ioInputContainer) {
        ...
        if (ioInputContainer.getRequestType() == IOInputContainerPIS.RequestType.INITIATE) {
            if (ioInputContainer.getIoParser().getOption(IO, ioInputContainer.getPaymentProduct().getValue()).getAsBoolean()) {
                ...
                ioInputContainer.setPaymentRequest(new PaymentInitiationPain001Request(PaymentProduct.byValue(IOInputContainerPIS.XML_PAYMENT_PRODUCT_PREFIX + ioInputContainer.getPaymentProduct().getValue()), PaymentService.PAYMENTS, document, ioInputContainer.getPsu()));
            } else {
                throw new StyxException(new ResponseEntity("The requested ASPSP does not support single-payments with payment-product " + ioInputContainer.getPaymentProduct().getValue(), ResponseConstant.BAD_REQUEST, ResponseCategory.ERROR, ResponseOrigin.ASPSP));
            }
