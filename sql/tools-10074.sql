--only allow oauth sca approach for sparda banks
UPDATE configs
SET config = '{
  "IO1": {
    "options": {
...
WHERE configs.id = (SELECT aspsps.config_id
                    FROM aspsps
                    WHERE aspsps.aspsp_group_id =
                          (SELECT aspsp_groups.id FROM aspsp_groups WHERE name ILIKE '%sparda%' LIMIT 1)
                    LIMIT 1);